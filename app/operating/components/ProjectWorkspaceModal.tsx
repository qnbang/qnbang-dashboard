...
function 날짜표시(value: string) {
  if (!value) return '수정 시각 미상';
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? value : date.toLocaleString('ko-KR', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
}
function 파일종류(file: 드라이브파일) {
  if (file.isFolder) return '폴더';
  if (file.mimeType.includes('spreadsheet')) return '시트';
  if (file.mimeType.includes('document')) return '문서';
  if (file.mimeType.includes('presentation')) return '프레젠테이션';
  if (file.mimeType.startsWith('image/')) return '이미지';
  if (file.mimeType.startsWith('video/')) return '영상';
...
export default function ProjectWorkspaceModal({ project, projects = [], tasks = [], editableProjectIds = [], writeMessage, onProjectUpdated, onTaskCreated, onTaskCompleted, onClose }: { project: 운영프로젝트; projects?: 운영프로젝트[]; tasks?: 운영할일[]; editableProjectIds?: string[]; writeMessage: string; onProjectUpdated: (project: 운영프로젝트) => void; onTaskCreated: (task: 운영할일) => void; onTaskCompleted: (task: 운영할일) => void; onClose: () => void }) {
  ...
  const [driveFiles, setDriveFiles] = useState<드라이브파일[]>([]);
  const [driveLoading, setDriveLoading] = useState(false);
  const [driveMessage, setDriveMessage] = useState('');
  const [fileSort, setFileSort] = useState<'최신순' | '이름순'>('최신순');
  const [fileSection, setFileSection] = useState('전체');
  ...
  const sortedLinks = useMemo(() => [...projectLinks].sort((a, b) => a.name.localeCompare(b.name, 'ko')), [projectLinks]);
  const fileSections = useMemo(() => ['전체', ...Array.from(new Set(driveFiles.map((file) => file.section)))], [driveFiles]);
  const visibleFiles = useMemo(() => driveFiles
    .filter((file) => fileSection === '전체' || file.section === fileSection)
    .sort((a, b) => fileSort === '이름순' ? a.name.localeCompare(b.name, 'ko') : String(b.updatedAt).localeCompare(String(a.updatedAt))), [driveFiles, fileSection, fileSort]);

  const loadDriveFiles = async (projectId = active.id) => {
    if (!projectId) { setDriveFiles([]); setDriveMessage('운영원장이 연결되면 Drive 파일을 볼 수 있습니다.'); return; }
    setDriveLoading(true);
    setDriveMessage('');
    try {
      const response = await fetch(`/api/operating/project-assets?projectId=${encodeURIComponent(projectId)}`, { cache: 'no-store' });
      const data = await response.json();
      if (!response.ok || !data.ok) throw new Error(data.error || 'Drive 파일을 읽지 못했습니다.');
      setDriveFiles(Array.isArray(data.files) ? data.files : []);
      setDriveMessage(data.message || '');
    } catch (error) {
      setDriveFiles([]);
      setDriveMessage(error instanceof Error ? error.message : 'Drive 파일을 읽지 못했습니다.');
    } finally {
      setDriveLoading(false);
    }
  };

  useEffect(() => { void loadDriveFiles(active.id); }, [active.id]);
  ...
  const addFile = async () => {
    ...
    try {
      ...
      await loadDriveFiles(active.id);
    } catch (error) {
  ...
  const 링크목록 = sortedLinks.length ? <div className={styles.assetList}>{sortedLinks.map((link) => <a className={styles.assetRow} href={link.url} target="_blank" rel="noreferrer" key={`${link.name}-${link.url}`}><span><b>{link.name || '연결 자료'}</b><small>{link.purpose || '용도 미기록'}</small></span><strong>링크</strong></a>)}</div> : <p className={styles.empty}>아직 연결한 공유 링크가 없습니다.</p>;
  const 파일목록 = <>
    <div className={styles.assetToolbar}>
      <div className={styles.assetFilters}>{fileSections.map((section) => <button type="button" key={section} className={fileSection === section ? styles.filterActive : ''} onClick={() => setFileSection(section)}>{section}</button>)}</div>
      <label><span>정렬</span><select value={fileSort} onChange={(event) => setFileSort(event.target.value as typeof fileSort)}><option>최신순</option><option>이름순</option></select></label>
    </div>
    {driveLoading ? <p className={styles.empty}>Drive 파일을 불러오는 중입니다.</p> : visibleFiles.length ? <div className={styles.assetList}>{visibleFiles.map((file) => <a className={styles.assetRow} href={file.url} target="_blank" rel="noreferrer" key={file.id}><span><b>{file.name}</b><small>{file.section} · {파일종류(file)} · {날짜표시(file.updatedAt)}</small></span><strong>{file.isFolder ? '폴더' : '파일'}</strong></a>)}</div> : <p className={styles.empty}>{driveMessage || '선택한 범위에 표시할 Drive 파일이 없습니다.'}</p>}
  </>;
  ...
  const 할일영역 = <section className={styles.readSection}>
    <div className={styles.workstreamHead}>
      ...
      <button className={styles.secondary} onClick={() => writable ? setIsAddingTask((value) => !value) : setNotice(writeMessage)}>할 일 추가</button>
    </div>
    {!writable && <p className={styles.checkPermissionNotice}>현재 이 프로젝트 원장은 읽기 전용입니다. 편집 권한을 연결하면 체크 결과가 즉시 원장에 저장됩니다.</p>}
    ...
    {linkedTasks.length ? <div className={styles.overviewTaskList}>{linkedTasks.map((task) => <article className={styles.readWorkstream} key={`${task.ledgerProjectId || active.id || active.name}-${task.id || task.title}`}><button className={`${styles.checkButton} ${done.includes(task.title) ? styles.checked : ''}`} aria-label={`${task.title} 완료 처리`} title={!writable ? '원장 편집 권한 연결 필요' : undefined} disabled={!task.id || Boolean(completingId) || (Boolean(task.ledgerProjectId) && !writable)} onClick={() => void completeTask(task)}>{completingId === task.id ? '…' : done.includes(task.title) ? '✓' : ''}</button><div><b>{task.title}</b><p>{task.owner} · {done.includes(task.title) ? '완료 기록됨' : task.state}</p></div><Badge>{task.due}</Badge></article>)}</div> : <p className={styles.empty}>연결된 할 일이 없습니다.</p>}
  </section>;
  ...
  return <div className={styles.modalBackdrop} role="presentation" onMouseDown={onClose}><section className={styles.projectBrowse} role="dialog" aria-modal="true" aria-label={`${active.name} 프로젝트 열람`} onMouseDown={(event) => event.stopPropagation()}>
    ...
    <main className={styles.projectRead}><div className={styles.projectReadHead}><div><p>프로젝트 / {active.name}</p><h2>{active.name}</h2><div className={styles.projectStatusRow}><Badge>{편집상태(active) === '고객대기' ? '고객 대기' : 편집상태(active)}</Badge><span>{active.client} · 담당 {active.owner} · 확인 시점 {active.due || '미정'}</span></div></div><div><button className={styles.secondary} onClick={() => writable ? setIsEditingProject((value) => !value) : setNotice(writeMessage)}>{isEditingProject ? '수정 닫기' : '프로젝트 정보 수정'}</button>{active.driveUrl && <a className={styles.secondary} href={active.driveUrl} target="_blank" rel="noreferrer">Drive 폴더 열기</a>}<button className={styles.closeButton} onClick={onClose}>닫기</button></div></div>
      ...
      {tab === '개요' && <><section className={styles.readSummary}><article><h3>프로젝트 설명</h3><p>{active.summary || '프로젝트 목표와 합의한 범위를 운영원장에서 확인합니다.'}</p><small>목표 · 합의된 결과를 만들기 / 완료 기준 · 고객 또는 내부 승인</small></article><article><h3>다음 행동</h3><b>{active.next || '다음 행동 등록 필요'}</b><small>할 일 {linkedTasks.length}개와 연결됨</small></article></section>{할일영역}<section className={styles.readSection}><h3>진행 묶음</h3>{streams.length ? streams.map((stream) => <article className={styles.readWorkstream} key={stream.name}><div><b>{stream.name}</b><p>만들 결과 · {stream.outcome || '결과 정의 필요'}</p></div><Badge>{stream.status || '확인 필요'}</Badge><span>담당 {stream.owner || '미정'} · 다음 행동 {stream.next || '등록 필요'}</span></article>) : <p className={styles.empty}>연결된 진행 묶음이 없습니다.</p>}</section><section className={styles.readSection}><div className={styles.workstreamHead}><div><h3>문서 · 링크</h3><span>자주 쓰는 연결 자료를 이름순으로 보여줍니다.</span></div><button className={styles.secondary} onClick={() => setTab('문서·링크')}>전체 보기</button></div>{sortedLinks.length ? <div className={styles.assetList}>{sortedLinks.slice(0, 5).map((link) => <a className={styles.assetRow} href={link.url} target="_blank" rel="noreferrer" key={`${link.name}-${link.url}`}><span><b>{link.name || '연결 자료'}</b><small>{link.purpose || '용도 미기록'}</small></span><strong>링크</strong></a>)}</div> : <p className={styles.empty}>아직 연결한 공유 링크가 없습니다.</p>}</section><section className={styles.readSection}><div className={styles.workstreamHead}><div><h3>최근 활동</h3><span>결정·일정·작업 이력을 최근 순서로 봅니다.</span></div><button className={styles.secondary} onClick={() => setTab('활동')}>활동 전체</button></div>{activities.length ? activities.slice(0, 6).map((item, index) => <article className={styles.readWorkstream} key={`${item.kind}-${item.id || index}`}><div><b>{item.title}</b><p>{[item.detail, item.actor].filter(Boolean).join(' · ') || '추가 설명 없음'}</p></div><Badge>{item.kind}</Badge><span>{item.at || '시각 미기록'}</span></article>) : <p className={styles.empty}>프로젝트 운영원장에 아직 등록된 활동이 없습니다.</p>}</section></>}
      {tab === '할 일' && 할일영역}
      {tab === '문서·링크' && <section className={styles.readSection}><div className={styles.workstreamHead}><div><h3>문서 · 링크</h3><span>Drive 실제 파일과 운영원장의 연결 링크를 한 곳에서 봅니다.</span></div><div><button className={styles.secondary} disabled={assetSaving} onClick={() => { if (!writable) { setNotice(writeMessage); return; } setIsAddingLink((value) => !value); setIsAddingFile(false); }}>링크 추가</button>{writable ? <button className={styles.primary} disabled={assetSaving || !active.driveUrl} onClick={() => { setIsAddingFile((value) => !value); setIsAddingLink(false); }}>파일 넣기</button> : active.driveUrl ? <a className={styles.primary} href={active.driveUrl} target="_blank" rel="noreferrer">Drive에 파일 넣기</a> : null}</div></div>{!writable && <p className={styles.permissionNotice}>대시보드 수정 권한이 없어서 Drive에서 직접 파일을 넣을 수 있도록 연결했습니다.</p>}{isAddingLink && <div className={styles.inlineAssetForm}><input value={linkForm.name} onChange={(event) => setLinkForm((current) => ({ ...current, name: event.target.value }))} placeholder="링크 이름" /><input value={linkForm.purpose} onChange={(event) => setLinkForm((current) => ({ ...current, purpose: event.target.value }))} placeholder="용도" /><input value={linkForm.url} onChange={(event) => setLinkForm((current) => ({ ...current, url: event.target.value }))} placeholder="https://" /><button className={styles.primary} disabled={assetSaving || !linkForm.name.trim() || !/^https?:\/\//i.test(linkForm.url)} onClick={() => void addLink()}>{assetSaving ? '기록 중' : '링크 기록'}</button></div>}{isAddingFile && <div className={styles.inlineAssetForm}><select value={fileForm.target} onChange={(event) => setFileForm((current) => ({ ...current, target: event.target.value as typeof current.target }))}><option>01_자료</option><option>02_제작</option><option>03_결과물</option></select><input type="file" onChange={(event) => setFileForm((current) => ({ ...current, file: event.target.files?.[0] }))} /><button className={styles.primary} disabled={assetSaving || !fileForm.file} onClick={() => void addFile()}>{assetSaving ? '업로드 중' : 'Drive에 넣기'}</button><small>50MB가 넘는 파일은 Drive 폴더에 직접 넣습니다.</small></div>}{notice && <p className={styles.empty}>{notice}</p>}<div className={styles.assetGroup}><div className={styles.assetGroupHead}><h4>Drive 파일</h4><span>{driveFiles.length}건</span></div>{파일목록}</div><div className={styles.assetGroup}><div className={styles.assetGroupHead}><h4>연결 링크</h4><span>{sortedLinks.length}건</span></div>{링크목록}{streams.filter((stream) => stream.links).map((stream) => <a className={styles.assetRow} href={stream.links} target="_blank" rel="noreferrer" key={stream.name}><span><b>{stream.name}</b><small>진행 묶음 연결 자료</small></span><strong>링크</strong></a>)}</div></section>}
