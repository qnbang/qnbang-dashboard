...
function Inbox({ selected, onSelect }: { selected: typeof messages[number]; onSelect: (item: typeof messages[number]) => void }) { const [channel, setChannel] = useState('전체'); const visible = messages.filter((message) => channel === '전체' || message.channel === channel); return <><ScreenIntro crumb="수신함" title="통합 수신함" description="채널은 달라도, 원문과 연결 원장은 잃지 않고 한 번만 기록합니다."/><div className={styles.inboxSearch}><input placeholder="보낸 사람, 내용, 고객사로 검색" /><div>{['전체', '문서', '라크'].map((item) => <button key={item} className={channel === item ? styles.filterActive : ''} onClick={() => setChannel(item)}>{item === '전체' ? '전체 3' : item}</button>)}</div></div><div className={styles.inboxFigma}><section className={`${styles.panel} ${styles.inboxList}`}><p className={styles.listOrder}>새로 들어온 순</p>{visible.map((m) => <button className={`${styles.inboxRow} ${selected.time === m.time ? styles.selected : ''}`} key={m.time} onClick={() => onSelect(m)}><Badge>{m.channel}</Badge><div><b>{m.sender}</b><p>{m.body}</p></div><time>{m.time}</time></button>)}<p className={styles.inboxFoot}>원문 보관 · 연결 기록 {messages.length}건</p></section><section className={`${styles.panel} ${styles.inboxDetail}`}><Header title="원문 대화" action={selected.channel}/><p className={styles.meta}>{selected.customer} · {selected.project}</p><div className={styles.bubble}>{selected.body}</div><div className={styles.detailInfo}><p><b>연결 고객</b>{selected.customer}</p><p><b>연결 프로젝트</b>{selected.project}</p><p><b>추천 다음 행동</b>{selected.action}</p></div><button className={styles.primary}>할 일로 등록</button></section></div></> }

function TaskList({ projects: taskProjects, onOpenProject }: { projects: Project[]; onOpenProject: (name: string) => void }) { const [done, setDone] = useState<string[]>([]); const [projectFilter, setProjectFilter] = useState('전체'); const [selectedTitle, setSelectedTitle] = useState(tasks[0].title); const visible = tasks.filter((task) => projectFilter === '전체' || task.project === projectFilter); const selected = tasks.find((task) => task.title === selectedTitle) || visible[0] || tasks[0]; const toggle = (title: string) => setDone((current) => current.includes(title) ? current.filter((item) => item !== title) : [...current, title]); return <><div className={styles.filterBar}><div><Badge>{`오늘 ${visible.filter((task) => !done.includes(task.title)).length}`}</Badge><Badge>{`완료 ${done.length}`}</Badge><Badge>{`전체 ${tasks.length}`}</Badge></div><select aria-label="프로젝트 필터" value={projectFilter} onChange={(event) => setProjectFilter(event.target.value)}><option value="전체">프로젝트 전체</option>{taskProjects.map((project) => <option key={project.name} value={project.name}>{project.name}</option>)}</select></div><section className={styles.taskLayout}><div className={styles.panel}><div className={styles.panelLead}><div><h2>모든 할 일</h2><p>오늘 → 마감일 순 · 프로젝트는 오른쪽 배지로 구분합니다.</p></div></div>{visible.map((t) => <button className={`${styles.taskListRow} ${done.includes(t.title) ? styles.taskDone : ''} ${selected.title === t.title ? styles.taskSelected : ''}`} key={t.title} onClick={() => setSelectedTitle(t.title)}><span className={`${styles.checkButton} ${done.includes(t.title) ? styles.checked : ''}`} role="checkbox" aria-checked={done.includes(t.title)} onClick={(event) => { event.stopPropagation(); toggle(t.title); }}>{done.includes(t.title) ? '✓' : ''}</span><span><b>{t.title}</b><small>{t.owner} · {done.includes(t.title) ? '완료' : t.state}</small></span><span className={styles.taskProjectButton}>{t.project}</span><time>{t.due}</time></button>)}<div className={styles.waiting}><b>대기 중 3건</b><span>고객 연결·거래 분류·일정 시간 확인이 필요한 항목입니다.</span></div></div><aside className={`${styles.panel} ${styles.taskDetail}`}><div className={styles.panelLead}><div><h2>선택한 할 일</h2></div></div><h3>{selected.title}</h3><Badge>{selected.due}</Badge><div className={styles.taskConnection}><p><b>연결 프로젝트</b>{selected.project}</p><p><b>담당</b>{selected.owner}</p><p><b>원문·메모</b>{selected.project}에서 이어진 할 일입니다.</p></div><button className={styles.secondary} onClick={() => onOpenProject(selected.project)}>프로젝트 열기</button><button className={styles.primary} onClick={() => toggle(selected.title)}>{done.includes(selected.title) ? '완료 취소' : '완료 처리'}</button><small>이 상태는 프로젝트·고객·홈 화면에 같은 원장 기준으로 반영됩니다.</small></aside></section></> }
...
function Projects({ projects: rows, selected, onSelect, onOpenProject }: { projects: Project[]; selected: Project; onSelect: (p: Project) => void; onOpenProject: (p: Project) => void }) { return <section className={styles.projectsScreen}><div className={styles.panel}><div className={styles.panelLead}><div><h2>진행 중인 프로젝트</h2><p>프로젝트별 원장의 다음 행동과 상태를 한 줄로 확인합니다.</p></div><span>{rows.length}건</span></div><div className={styles.projectTableHead}><span>프로젝트</span><span>거래상대</span><span>진행률</span><span>다음 행동</span><span>확인 시점</span></div>{rows.map((p) => <button className={`${styles.projectTableRow} ${selected.name === p.name ? styles.selected : ''}`} onClick={() => { onSelect(p); onOpenProject(p); }} key={p.name}><span><b>{p.name}</b><small>{p.status} · 담당 {p.owner}</small></span><span>{p.client}</span><span className={styles.progressCell}><i><em style={{ width: `${p.progress}%` }}></em></i><strong>{p.progress}%</strong></span><span>{p.next}</span><time>{p.due || '미정'}</time></button>)}</div><aside className={`${styles.panel} ${styles.projectRegister}`}><div className={styles.panelLead}><div><h2>새 프로젝트 등록</h2><p>등록은 구글 드라이브 폴더와 프로젝트 원장을 함께 만듭니다.</p></div></div><label className={styles.field}><span>프로젝트 이름</span><input placeholder="예: 브랜드 웹사이트 개편" disabled /></label><label className={styles.field}><span>거래상대</span><input placeholder="고객 또는 자체 브랜드" disabled /></label><label className={styles.field}><span>담당</span><input placeholder="담당자 선택" disabled /></label><p className={styles.registerNotice}>저장 연동이 아직 준비되지 않아, 등록은 현재 프로젝트 원장에서 먼저 진행합니다.</p><button className={styles.secondary} disabled>등록 준비 중</button></aside></section> }
...
function ProjectBrowseModal({ project, onClose }: { project: Project; onClose: () => void }) { const [tab, setTab] = useState<'개요' | '할 일' | '문서·파일' | '활동'>('개요'); const workstreams = project.workstreams || []; return <div className={styles.modalBackdrop} role="presentation" onMouseDown={onClose}><section className={styles.projectBrowse} role="dialog" aria-modal="true" aria-label={`${project.name} 프로젝트 열람`} onMouseDown={(event) => event.stopPropagation()}><aside className={styles.projectExplorer}><div><h2>프로젝트</h2><input placeholder="프로젝트 검색" /><p>열람 필터</p><div className={styles.explorerChips}><Badge>진행 중</Badge><Badge>내 담당</Badge><Badge>마감 임박</Badge></div></div><div className={styles.explorerList}>{projects.map((item) => <div className={item.name === project.name ? styles.explorerActive : ''} key={item.name}><b>{item.name}</b><small>{item.due || '확인 시점 미정'} · {item.next}</small></div>)}</div><button className={styles.primary} onClick={onClose}>목록으로 돌아가기</button></aside><main className={styles.projectRead}><div className={styles.projectReadHead}><div><p>프로젝트 / {project.name}</p><h2>{project.name}</h2><span><Badge>{project.status}</Badge>{project.client} · 담당 {project.owner} · 확인 시점 {project.due || '미정'}</span></div><button className={styles.closeButton} onClick={onClose}>닫기</button></div><nav className={styles.projectTabs}>{(['개요','할 일','문서·파일','활동'] as const).map((item) => <button key={item} className={tab === item ? styles.tabActive : ''} onClick={() => setTab(item)}>{item}{item === '할 일' ? ` ${workstreams.length}` : ''}</button>)}</nav>{tab === '개요' && <><section className={styles.readSummary}><article><h3>프로젝트 설명</h3><p>{project.summary || '프로젝트 목표와 합의한 범위를 운영원장에서 확인합니다.'}</p><small>목표 · 합의된 결과를 만들기 / 완료 기준 · 고객 또는 내부 승인</small></article><article><h3>다음 행동</h3><b>{project.next}</b><small>진행 묶음과 할 일에서 이어서 확인</small></article></section><section className={styles.readSection}><h3>진행 묶음</h3>{workstreams.length ? workstreams.map((stream) => <article className={styles.readWorkstream} key={stream.name}><div><b>{stream.name}</b><p>만들 결과 · {stream.outcome}</p></div><Badge>{stream.status}</Badge><span>담당 {stream.owner} · 다음 행동 {stream.next}</span></article>) : <p>아직 등록된 진행 묶음이 없습니다.</p>}</section></>}{tab === '할 일' && <section className={styles.readSection}><h3>연결된 할 일</h3>{tasks.filter((task) => task.project === project.name).map((task) => <article className={styles.readWorkstream} key={task.title}><div><b>{task.title}</b><p>{task.owner} · {task.state}</p></div><span>{task.due}</span></article>)}</section>}{tab === '문서·파일' && <section className={styles.readSection}><h3>문서·파일 링크</h3><p>실제 파일은 구글 드라이브에 두고, 프로젝트 원장의 링크 탭에서 필요한 자료만 연결합니다.</p></section>}{tab === '활동' && <section className={styles.readSection}><h3>최근 활동</h3><p>결정·일정·이력은 프로젝트 운영원장에서 같은 시간순으로 확인합니다.</p></section>}</main></section></div> }
...
function ProjectFigmaModal({ project, onClose }: { project: Project; onClose: () => void }) {
  const linkedTasks = tasks.filter((task) => task.project === project.name);
  const streams = project.workstreams || [];
  return <div className={styles.modalBackdrop} role="presentation" onMouseDown={onClose}><section className={styles.projectBrowse} role="dialog" aria-modal="true" aria-label={`${project.name} 프로젝트 열람`} onMouseDown={(event) => event.stopPropagation()}><aside className={styles.projectExplorer}><div><h2>프로젝트</h2><input placeholder="프로젝트 검색" /><p>열람 필터</p><div className={styles.explorerChips}><Badge>진행 중</Badge><Badge>내 담당</Badge><Badge>마감 임박</Badge></div></div><div className={styles.explorerList}>{projects.map((item) => <div className={item.name === project.name ? styles.explorerActive : ''} key={item.name}><b>{item.name}</b><small>{item.due || '확인 시점 미정'} · {item.next}</small></div>)}</div><button className={styles.primary} onClick={onClose}>목록으로 돌아가기</button></aside><main className={styles.projectRead}><div className={styles.projectReadHead}><div><p>프로젝트 / {project.name}</p><h2>{project.name}</h2><span><Badge>{project.status}</Badge>{project.client} · 담당 {project.owner} · 확인 시점 {project.due || '미정'}</span></div><div><button className={styles.secondary}>Drive 폴더 열기</button><button className={styles.closeButton} onClick={onClose}>닫기</button></div></div><nav className={styles.projectTabs}><button className={styles.tabActive}>개요</button><button>할 일 {linkedTasks.length}</button><button>문서·파일</button><button>활동</button></nav><section className={styles.readSummary}><article><h3>프로젝트 설명</h3><p>{project.summary || '프로젝트 목표와 합의한 범위를 운영원장에서 확인합니다.'}</p><small>목표 · 합의된 결과를 만들기 / 완료 기준 · 고객 또는 내부 승인</small></article><article><h3>다음 행동</h3><b>{project.next}</b><small>할 일 {linkedTasks.length}개와 연결됨</small></article></section><section className={styles.readSection}><div className={styles.workstreamHead}><div><h3>할 일</h3><span>프로젝트 할 일은 캘린더와 내 할 일에서 같은 기록으로 보입니다.</span></div><button className={styles.secondary}>할 일 추가</button></div>{linkedTasks.length ? linkedTasks.map((task) => <article className={styles.readWorkstream} key={task.title}><span className={styles.checkButton} role="checkbox" aria-checked="false" /><div><b>{task.title}</b><p>{task.owner} · {task.state}</p></div><Badge>{task.due}</Badge></article>) : <p>연결된 할 일이 없습니다. 다음 행동을 할 일로 추가해 주세요.</p>}</section><section className={styles.readSection}><div className={styles.workstreamHead}><div><h3>문서 · 링크 · 파일</h3><span>구글 드라이브 프로젝트 폴더와 연결됨</span></div><div><button className={styles.secondary}>링크 추가</button><button className={styles.primary}>파일 넣기</button></div></div>{streams.length ? streams.map((stream) => <article className={styles.readWorkstream} key={stream.name}><div><b>{stream.name}</b><p>만들 결과 · {stream.outcome || '결과 정의 필요'}</p></div><Badge>{stream.status || '확인 필요'}</Badge></article>) : <p>연결 자료는 프로젝트 운영원장의 링크 탭에서 추가합니다.</p>}</section><section className={styles.readSection}><h3>최근 활동</h3><p>오늘 10:42 · 프로젝트 정보가 통합 수신함과 연결됨</p><p>오늘 09:30 · 다음 행동이 할 일 원장에 반영됨</p></section></main></section></div>;
}
