...
type View = '홈' | '수신함' | '할 일' | '캘린더' | '거래상대 관리' | '프로젝트' | '재무·정산' | '공용 도구' | '운영 설정';
...
const menu: View[] = ['홈', '수신함', '할 일', '캘린더', '거래상대 관리', '프로젝트', '재무·정산', '공용 도구', '운영 설정'];
const tasks = [
...
export default function OperatingPage() {
  ...
  const content = () => {
    ...
    if (view === '거래상대 관리') return <Partners query={query} />;
...
type Partner = { id: string; name: string; kind: '법인·기관' | '개인'; relation: '고객' | '잠재 고객' | '협력자' | '소개자'; projects: string[]; last: string; next: string; contacts: { name: string; role: string; phone?: string; email?: string; card?: string }[]; note: string };
const initialPartners: Partner[] = [
  { id: 'p-darimati', name: '다리마티', kind: '법인·기관', relation: '고객', projects: ['다리마티 운동회'], last: '8월 7일 · 라크', next: '시그니처 종목·공간·예산 구조 우선순위 확정', contacts: [{ name: '황현욱', role: '프로젝트 담당', card: '명함 등록됨' }], note: '첫 행사는 10월 18일로 확정되었습니다.' },
  { id: 'p-vq', name: '브이큐스튜디오', kind: '법인·기관', relation: '고객', projects: ['브이큐 업무자동화'], last: '8월 5일 · 문서', next: '제안 문서와 체크리스트 현황 확인', contacts: [{ name: '손정현', role: '대표', card: '명함 등록됨' }], note: '업무자동화 제안 문서와 체크리스트를 함께 확인 중입니다.' },
  { id: 'p-good', name: '좋은움직임연구소', kind: '법인·기관', relation: '고객', projects: ['좋은움직임연구소 러너 세션'], last: '7월 31일 · 문서', next: '납품 문서 4종을 대표님과 검토', contacts: [{ name: '오승식', role: '대표', email: 'contact@movementlab.kr' }], note: '60분 러너 세션의 표현·홍보·고객관리 문서를 묶어 검토합니다.' },
  { id: 'p-mangwon', name: '망리단길골목형상점가 상인회', kind: '법인·기관', relation: '고객', projects: ['망원 야간보물찾기'], last: '최신 기록 확인 필요', next: '행사 개발·현장테스트 일정 재확인', contacts: [{ name: '상인회 담당자', role: '운영 담당' }], note: '행사 일정과 참여 매장 명단을 다시 확인해야 합니다.' },
  { id: 'p-referral', name: '김민지', kind: '개인', relation: '소개자', projects: [], last: '8월 2일 · 명함', next: '소개 건 진행 여부 확인', contacts: [{ name: '김민지', role: '개인 소개자', card: '명함 등록됨' }], note: '고객사로 전환되기 전에도 관계와 명함을 보관합니다.' },
];

function Partners({ query }: { query: string }) {
  const [partners, setPartners] = useState<Partner[]>(initialPartners);
  const [selectedId, setSelectedId] = useState(initialPartners[0].id);
  const [filter, setFilter] = useState<'전체' | Partner['kind'] | Partner['relation']>('전체');
  const [isRegistering, setIsRegistering] = useState(false);
  const [cardFile, setCardFile] = useState<File | null>(null);
  const [form, setForm] = useState({ partnerId: 'new', partnerName: '', kind: '법인·기관' as Partner['kind'], relation: '잠재 고객' as Partner['relation'], contactName: '', role: '', phone: '', email: '' });
  const rows = partners.filter((partner) => (filter === '전체' || partner.kind === filter || partner.relation === filter) && `${partner.name} ${partner.contacts.map((contact) => contact.name).join(' ')}`.includes(query));
  ...
  const register = () => {
    ...
    const contact = { name: contactName, role: form.role.trim() || (form.kind === '개인' ? '개인 거래상대' : '담당자'), phone: form.phone.trim() || undefined, email: form.email.trim() || undefined, card: cardFile?.name || '명함 정보 직접 등록' };
    if (form.partnerId !== 'new') {
      ...
      const id = `p-${Date.now()}`;
      setPartners((current) => [...current, { id, name: partnerName, kind: form.kind, relation: form.relation, projects: [], last: '방금 명함 등록', next: '첫 대화와 다음 행동 등록', contacts: [contact], note: '명함에서 새로 등록한 거래상대입니다.' }]);
      setSelectedId(id);
    }
    setCardFile(null);
    setForm({ partnerId: 'new', partnerName: '', kind: '법인·기관', relation: '잠재 고객', contactName: '', role: '', phone: '', email: '' });
    setIsRegistering(false);
  };
  return <>
    <section className={styles.partnerToolbar}><div><h2>거래상대 관리</h2><p>고객사·개인·소개자·협력자를 한 번만 등록하고, 프로젝트와 담당자에 연결합니다.</p></div><button className={styles.primary} onClick={() => setIsRegistering(true)}>명함 등록</button></section>
    <section className={styles.partnerFilters}>{(['전체', '법인·기관', '개인', '고객', '잠재 고객', '협력자', '소개자'] as const).map((item) => <button key={item} className={filter === item ? styles.filterActive : ''} onClick={() => setFilter(item)}>{item}</button>)}</section>
    <section className={styles.partnerLayout}><div className={styles.panel}><Header title="거래상대 목록" action={`${rows.length}건`} />{rows.length ? <div className={styles.partnerList}>{rows.map((partner) => <button className={`${styles.partnerRow} ${selected.id === partner.id ? styles.selected : ''}`} key={partner.id} onClick={() => setSelectedId(partner.id)}><div><b>{partner.name}</b><small>{partner.kind} · {partner.relation} · 담당자 {partner.contacts.length}명</small></div><span><Badge>{partner.projects.length ? `${partner.projects.length}개 프로젝트` : '프로젝트 미연결'}</Badge><small>{partner.last}</small></span></button>)}</div> : <p className={styles.empty}>조건에 맞는 거래상대가 없습니다.</p>}</div>
      <div className={`${styles.panel} ${styles.partnerDetail}`}><Header title={selected.name} action={`${selected.kind} · ${selected.relation}`} /><p className={styles.meta}>{selected.note}</p><div className={styles.partnerSection}><h3>연결 담당자와 명함</h3>{selected.contacts.map((contact) => <div className={styles.contactRow} key={contact.name}><div><b>{contact.name}</b><small>{contact.role}{contact.phone ? ` · ${contact.phone}` : ''}{contact.email ? ` · ${contact.email}` : ''}</small></div>{contact.card ? <Badge>{contact.card === '명함 등록됨' ? '명함 있음' : '등록됨'}</Badge> : <span className={styles.noCard}>명함 없음</span>}</div>)}</div><div className={styles.partnerSection}><h3>연결 프로젝트</h3>{selected.projects.length ? selected.projects.map((project) => <div className={styles.linkRow} key={project}><b>{project}</b><span>프로젝트 원장에서 진행 상황 확인</span></div>) : <p className={styles.empty}>아직 연결한 프로젝트가 없습니다.</p>}</div><div className={styles.partnerSection}><h3>관계 기록</h3><div className={styles.timeline}><p><b>최근 접점</b>{selected.last}</p><p><b>다음 행동</b>{selected.next}</p></div></div></div>
    </section>
    {isRegistering && <div className={styles.modalBackdrop} role="presentation"><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="card-register-title"><div className={styles.modalHead}><div><h2 id="card-register-title">명함 등록</h2><p>사진을 올리고, 연결할 거래상대와 담당자만 확인하면 됩니다.</p></div><button className={styles.closeButton} aria-label="명함 등록 닫기" onClick={() => setIsRegistering(false)}>닫기</button></div><div className={styles.formGrid}><label className={styles.fileField}><span>명함 사진</span><input type="file" accept="image/*" onChange={(event) => setCardFile(event.target.files?.[0] || null)} /><strong>{cardFile ? cardFile.name : '사진 선택'}</strong><small>자동 추출 연동 전에는 사진과 정보를 함께 보관합니다.</small></label><label className={styles.field}><span>연결할 거래상대</span><select value={form.partnerId} onChange={(event) => updateForm('partnerId', event.target.value)}><option value="new">새 거래상대 만들기</option>{partners.map((partner) => <option key={partner.id} value={partner.id}>{partner.name}</option>)}</select></label>{form.partnerId === 'new' && <><label className={styles.field}><span>거래상대 이름</span><input value={form.partnerName} onChange={(event) => updateForm('partnerName', event.target.value)} placeholder="회사명 또는 개인 이름" /></label><label className={styles.field}><span>관계</span><select value={form.relation} onChange={(event) => updateForm('relation', event.target.value)}><option>잠재 고객</option><option>고객</option><option>협력자</option><option>소개자</option></select></label><label className={styles.field}><span>구분</span><select value={form.kind} onChange={(event) => updateForm('kind', event.target.value)}><option>법인·기관</option><option>개인</option></select></label></>}<label className={styles.field}><span>담당자 이름</span><input value={form.contactName} onChange={(event) => updateForm('contactName', event.target.value)} placeholder="명함에 적힌 이름" /></label><label className={styles.field}><span>직함 또는 역할</span><input value={form.role} onChange={(event) => updateForm('role', event.target.value)} placeholder="예: 대표, 프로젝트 담당" /></label><label className={styles.field}><span>전화번호</span><input value={form.phone} onChange={(event) => updateForm('phone', event.target.value)} placeholder="선택 입력" /></label><label className={styles.field}><span>이메일</span><input value={form.email} onChange={(event) => updateForm('email', event.target.value)} placeholder="선택 입력" /></label></div><div className={styles.modalFooter}><button className={styles.secondary} onClick={() => setIsRegistering(false)}>취소</button><button className={styles.primary} disabled={!(form.contactName.trim() && (form.partnerId !== 'new' || form.partnerName.trim()))} onClick={register}>등록하기</button></div></section></div>}
  </>;
}

function Projects({ projects: rows, selected, onSelect }: { projects: Project[]; selected: Project; onSelect: (p: Project) => void }) { const workstreams = selected.workstreams || []; return <div className={styles.projectLayout}><section className={styles.panel}><Header title="프로젝트 목록" action={`${rows.length}건`} />{rows.map((p) => <button className={`${styles.projectSelect} ${selected.name === p.name ? styles.selected : ''}`} onClick={() => onSelect(p)} key={p.name}><span><b>{p.name}</b><small>{p.client} · 담당 {p.owner}</small><small>다음 행동 · {p.next}</small></span><Badge>{p.status}</Badge><strong>{p.progress}%</strong></button>)}</section><section className={`${styles.panel} ${styles.projectDetail}`}><Header title={selected.name} action={selected.status}/><p className={styles.meta}>{selected.client} · 담당 {selected.owner} · 확인 시점 {selected.due || '미정'}</p><div className={styles.projectSnapshot}><p><b>목표</b>{selected.summary || '프로젝트 목표와 합의된 범위를 원장에서 확인합니다.'}</p><p><b>막힌 점</b>{selected.blocker || '현재 등록된 막힘 없음'}</p><p><b>다음 행동</b>{selected.next}</p></div><div className={styles.workstreamHead}><h3>진행 묶음</h3><span>결과를 만드는 업무 단위</span></div>{workstreams.length ? <div className={styles.workstreamList}>{workstreams.map((stream) => <article className={styles.workstream} key={stream.name}><div><b>{stream.name}</b><small>만들 결과 · {stream.outcome || '결과 정의 필요'}</small></div><Badge>{stream.status || '상태 확인 필요'}</Badge><p><span>담당</span>{stream.owner || '미정'}</p><p><span>다음 행동</span>{stream.next || '다음 행동 등록 필요'}</p>{stream.due && <p><span>확인 시점</span>{stream.due}</p>}{stream.links && <p><span>자료 링크</span><a href={stream.links} target="_blank" rel="noreferrer">연결 자료 열기</a></p>}</article>)}</div> : <p className={styles.empty}>아직 진행 묶음이 없습니다. 여러 할 일이 이어져 결과를 만들 때만 추가합니다.</p>}<div className={styles.projectFootnote}><span>할일 · 결정 · 일정 · 이력 · 링크는 프로젝트 원장에서 이어서 확인합니다.</span><span>드라이브에는 파일만 두고, 이 화면은 필요한 링크만 표시합니다.</span></div>{selected.hubUrl ? <Link className={styles.primary} href={selected.hubUrl}>협업 허브 열기</Link> : <button className={styles.primary}>프로젝트 열기</button>}</section></div> }
