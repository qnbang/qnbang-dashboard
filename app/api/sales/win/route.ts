...
const 과업H = ['id', '프로젝트', '과업명', '담당자', '공위치', '현재상태', '다음할일', '기한', '고객', '돈종류', '할일', '판정근거', '갱신일', '출처'];
const 매출H = ['계약일', '계약명', '클라이언트', '계약금액', '부가세', '입금상태', '입금일', '입금액', '순매출', '비고'];
...
export async function POST(req: Request) {
  try {
    ...
    const 과업res = await post({ key: KEY, 종류: '과업', 헤더: 과업H, 행: {
      ...
      할일: '', 판정근거: '', 갱신일: today, 출처: '영업전환',
    }, 드롭다운: { 공위치: ['시작전', '내작업', '내회신', '고객대기', '보류', '완수'], 돈종류: ['매출', '투자'] } });

    // 2. 매출 원장 등록(입금대기). 금액 미정이면 비움.
    const 매출res = await post({ key: KEY, 종류: '매출', 헤더: 매출H, 행: {
      계약일: today, 계약명: 대상, 클라이언트: 대상, 계약금액: 금액 || '', 부가세: '',
      입금상태: '입금대기', 입금일: '', 입금액: 0, 순매출: 금액 || '', 비고: '영업보드 계약전환 ' + today,
    } });
