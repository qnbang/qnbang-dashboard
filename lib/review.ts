...
const pathOf = (key: string) => `reviews/${safeKey(key)}.json`;

// 저장된 검토 읽기 → 데이터 + 쓰기용 sha. 없으면 빈 데이터.
export async function getReview(key: string): Promise<{ data: ReviewData; sha?: string }> {
  const { data, sha } = await getJsonFile<ReviewData>(pathOf(key));
  return { data: { items: data?.items || {}, updatedAt: data?.updatedAt || '' }, sha };
}
...
export async function saveReview(key: string, items: ReviewData['items'], nowIso: string): Promise<ReviewData> {
  ...
  await saveJsonFile(pathOf(key), data, { sha, message: `검토 저장: ${safeKey(key)}` });
  return data;
}
